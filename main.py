...
class ConfigGenerator:
    def __init__(self):
        self.templates = {
            'vless': self.generate_vless,
            'vmess': self.generate_vmess,
            'trojan': self.generate_trojan,
            'shadowsocks': self.generate_shadowsocks,
            'socks': self.generate_socks,
            'http': self.generate_http_config,
            'ssh': self.generate_ssh,
            'wireguard': self.generate_wireguard
        }
    
    def generate_vless(self, server, port, uuid_val, network='ws', path='/'):
        config = {"protocol": "vless", "settings": {"vnext": [{"address": server, "port": port, "users": [{"id": uuid_val, "encryption": "none", "level": 0}]}]}, "streamSettings": {"network": network, "wsSettings": {"path": path}}}
    ...
    def generate_http_config(self, host, port, payload=''):
    ...
    def generate_ssh(self, host, port, username, password):
        return json.dumps({"host": host, "port": port, "username": username, "password": password, "payload": f"GET / HTTP/1.1[crlf]Host: {host}[crlf][crlf]"})
...
def generate_config():
    ...
    if protocol in config_gen.templates:
